...
func newStateSnapshotIterator(snapshot *gorocksdb.Snapshot) (*StateSnapshotIterator, error) {
	...
	dbItr := openchainDB.GetStateSnapshotIterator(snapshot)
...
func (snapshotItr *StateSnapshotIterator) GetRawKeyValue() ([]byte, []byte) {
	...
	keyBytes := statemgmt.Copy(snapshotItr.dbItr.KeyData())
	valueBytes := statemgmt.Copy(snapshotItr.dbItr.ValueData())
